...
# 
# ggplot() +
#   geom_sf(data = ward_map, fill = NA, color = "black") +
...
# 
# 
# st_geometry_type(ward_map)


######################################################################################

chronic_liver_disease <- read_excel("chronic liver disease.xlsx")
...
#          local_pop = winsorize(local_pop),
#          age_specific_rate = count / local_pop,
#          age_standardised_count = age_specific_rate * Standard_Population,
#          age_standardised_count_squared = (Standard_Population / local_pop)^2 * count)
# 
# 
# 
# warddata = warddata%>%
...
#     age_standardised_count_squared = sum(age_standardised_count_squared),
#     .groups = 'drop'
#   )%>%
#   mutate(
#     DSR = (age_standardised_count / Standard_Population) * 100000, # Expressed per 100,000
#     Var_DSR = age_standardised_count_squared / (Standard_Population^2),
#     SE_DSR = sqrt(Var_DSR) * 100000 # Adjusted for per 100,000
#   )
# 
# 
# ############################################
...
# 
# warddata = warddata %>% 
#   left_join(imd_ward, by = c("Ward" = "Area_Code")) 
# 
# ############################################
# #merge with the map 
# 
# 
# ward_map<-ward_map %>%
#   left_join(warddata, by = c("WD21CD"="Ward"))  # Ensure correct column match
# 
# 
# 
# ###########################################
# #define the variables to biscale 
# datafornimap = bi_class(ward_map, x = DSR, y = IMD, style = "quantile", dim = 4)
# 
# 
# #plot the map 
# plot = ggplot() +
#   geom_sf(data = datafornimap, mapping = aes(fill = bi_class), color = "white", size = 0.1, show.legend = FALSE) +
#   bi_scale_fill(pal = "GrPink2", dim = 4) +
#   labs(
#     title = "Mortality rate of chronic liver disease in 24/25 and IMD score"
#   ) +
